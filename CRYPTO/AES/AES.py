...
def inv_subBytes(aesState):
    ...
    for i in range(0, len(aesState), 4):
        #print(i)
        #print(aesState[i:i+4])
        #print(resState[i:i+4])
        resState[i:i + 4] = inv_subWord(aesState[i:i + 4])
        #resState.append(inv_subWord(aesState[i:i + 4]))
        #print(resState)
...
def inv_shiftRow(aesState):
    resState = bytearray()
    newPosition = [0x00,0x0d,0x0a,0x07,0x04,0x01,0x0e,0x0b,0x08,0x05,0x02,0x0f,0x0c,0x09,0x06,0x03]
    for i in newPosition:
        resState.append(aesState[i])
